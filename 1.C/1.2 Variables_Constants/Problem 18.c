...
int isTrue(char cIndex, int endOfCycle()) {
	if (cIndex == 'a')
	{
		printf("Cancelling all orders..\n");
		return 1;
	}
	else if (cIndex == 'y')
	{
...
int main()
{
	...
	do
	{
		...
		{
			if (iCaravan_Count == 0 && iCamper_Count == 0)
			{  // printf("\nCurrent bill = %d lv.\n", iSum_Caravan + iSum_Camper); // added last
				printf("No more vehilces left for rent! Proceed to check out? [Yes = 'y'] [Abort = 'a'] : ");
				scanf(" %c", &cIndex);
				
				if (isTrue(cIndex, endOfCycle(cIndex, arr_Caravan, arr_Camper, iPrice_Per_Caravan_Per_Day, iPrice_Per_Camper_Per_Day, iSum_Caravan, iSum_Camper)))
				{
					return 0;
				}
				/*if (cIndex == 'a')
		...
		if (iCaravan_Count == 0)
		{
			do
			{
				printf("Vehilcles available for rent [Camper = 2] : "); //1
