...
int main(void)
{
	...
	char user_selection[5] = { '\0' };
		...
		for (; *user_selection != 'q';)
		{
			...
			{
				*user_selection = '\0';
				choose_brand_menu(model_names(modelNames, cars, STRUCT_SIZE_CARS), cars, STRUCT_SIZE_CARS, user_selection); //accept node (identofocator ndavailability) instead of cars ?
	...
	// Ex 5 a____________________________________
	int days = 100;
	...
	final_date = date_variance_v2(startDate, days);
	print_time(&final_date);

	// Ex 5 b____________________________________
	datetime date_begin =
	{
		.day_of_month = 20,
		.month = December,
	...
	// Ex 6 b____________________________________
	char buffer[50];

	puts("\n--------------------------------------------");
	printf("%s",my_strftime(&date_end, buffer));
